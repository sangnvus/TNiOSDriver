...
//NSString *userName = [];
-(void)showDriverDetailsToEditingWithFname:(NSString *)firstName withLname:(NSString *)lastName withEmail:(NSString *)email withPhoneNo:(NSString *)phoneNo
{
...
-(void)showDriverDetailsReadOnlyWithFname:(NSString *)firstName withLname:(NSString *)lastName withEmail:(NSString *)email withPhoneNo:(NSString *)phoneNo withBalance:(NSString *)balance
{
...
- (void)viewDidLoad {
    ...
    if ([appDelegate.profileFlag isEqualToString:@"0"]) {
        a2 = [NSString stringWithFormat:@"%@",[appDelegate.yoursefl objectForKey:@"balance"]];
        ...
        a6 = [appDelegate.yoursefl objectForKey:@"phoneNumber"];
        NSLog(@"vao%@ -phone", appDelegate.profileFlag);
    ...
    countBtnClicked = 0;
    [self showDriverDetailsReadOnlyWithFname:a3
                                  withLname:a4
                                  withEmail:a5
                                withPhoneNo:a6 withBalance:a2];
    // to dimis keyboard
...
-(IBAction)editProfile:(id)sender {
    ...
    if (countBtnClicked==0) {
        [self showDriverDetailsToEditingWithFname:a3 withLname:a4 withEmail:a5 withPhoneNo:a6];
    }
    if(countBtnClicked>0){
        NSLog(@"COUNT on if:%d",countBtnClicked);
        [self showDriverDetailsReadOnlyWithFname:self.firstNameField.text
                                      withLname:self.lastNameField.text
                                      withEmail:self.emailField.text
                                    withPhoneNo:self.phoneNoField.text withBalance:self.balanceLb.text];
        [self showDriverDetailsToEditingWithFname:self.firstNameField.text
                                       withLname:self.lastNameField.text
                                       withEmail:self.emailField.text
...
- (IBAction)doneEditProfile:(id)sender {
    ...
    if ( [self.firstNameField.text isEqualToString:@""])
    {
        ...
        {
            UIAlertView *alertTmp =[[UIAlertView alloc]initWithTitle:@""
                                                             message:NSLocalizedString(@"please input correct email",nil)
                                                            delegate:self
                                                   cancelButtonTitle:NSLocalizedString(@"OK",nil)
                                                   otherButtonTitles:nil, nil];
            [alertTmp show];
        ...
        else
        {
            if( [self.phoneNoField.text isEqualToString:@""]|| self.phoneNoField.text==nil)
            {
                ...
                NSString *driverId = [NSString stringWithFormat:@"%@",[userData objectForKey:@"idDriver"]];
                NSString *data = [NSString stringWithFormat:   @"{\"id\":\"%@\",\"firstname\":\"%@\",\"lastname\":\"%@\",\"phoneNumber\":\"%@\",\"email\":\"%@\"}",driverId,self.firstNameField.text,self.lastNameField.text,self.phoneNoField.text,self.emailField.text];
                ...
                [unity updateByDriverById:base64String owner:self];
